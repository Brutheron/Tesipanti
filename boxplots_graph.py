...
df = df.loc[
    [(list(hue_values_dict.keys())[0], "1.png")]
    + [(i, "2.png") for i in list(hue_values_dict.keys())[1:]]
]
df = df.dropna(axis=1)
df = df.transpose()
...
box_plot = sns.boxplot(
    ax=ax,
    data=[df.iloc[:, x].values for x in range(len(df.columns))],
    palette="Set3",
    orient="h",
)
...
yticks = ["Antes"] + [f"MB: {df.columns[x+1][0]/10}" for x in range(len(df.columns[1:]))]
